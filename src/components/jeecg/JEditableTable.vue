                ...
                <div v-if="dragSort"
                  ...
                  <a-dropdown :trigger="['click']"
                    ...
                    <a-menu slot="overlay">
                      <a-menu-item key="0"
                                   :disabled="rowIndex===0"
                                   @click="_handleRowMoveUp(rowIndex)">向上移
                      </a-menu-item>
                      <a-menu-item key="1"
...
export default {
  ...
  methods: {
    ...
    /** popup回调 */
    popupCallback: function (value, others, id, row, column, index) {
      let idPreFix = id.split('_')[0]
      let _values = value.split(',')
      //要增加的行数
      let preAdd = _values.length - 1
      //假设操作为最后一行
      var rowIndex = this.rows.length - 1
      this.rows.forEach((item, _index) => {
        if (item.id == row.id) {
          rowIndex = _index
        }
      })
      //要增加的行数
      if (this.rows.length - 1 - rowIndex < preAdd) {
        this.add(preAdd - this.rows.length + 1 + rowIndex)
      }
      //把属性和值拆分存储
      let _key = []
      let _value = []
      for (const othersKey in others) {
        _key.push(othersKey)
        _value.push(others[othersKey].split(','))
      }
      //把拆分的属性和值重新组装成对象
      for (let j = 0; j < _values.length; j++) {
        let item = {}
        for (let i = 0; i < _key.length; i++) {
          item[_key[i]] = _value[i][j]
        }
        this.myPopupCallback(_values[j], item, idPreFix + this.rows[index].id, this.rows[index], column, index++)
      }
    },
    myPopupCallback (value, others, id, row, column, index) {
      // 存储输入的值
